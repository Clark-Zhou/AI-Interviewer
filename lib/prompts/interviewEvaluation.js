 ...
 *
 * 关联文件：
 * - lib/server/interviewEvaluation.js：后续会调用这里的 prompt 并发送给 DeepSeek。
 * - lib/server/parseInterviewEvaluation.js：后续会按这里约定的 JSON 结构解析 AI 返回。
 * - app/api/evaluate-interview/route.js：后续会通过服务层间接使用这里的 prompt。
 *
 * 说明：
...
export function buildInterviewEvaluationPrompt({ jobInfo, resume, questionAnswers }) {
  return `
