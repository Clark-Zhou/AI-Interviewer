...
当前版本先验证最小可用闭环。当前已完成本地历史记录初始化、列表和详情查看、登录入口页面壳、`/login` 和 `/interview` 路由拆分、Supabase Auth 账号系统 MVP，以及内部测试版上线准备文档。下一步计划补一个基础主页框架：根路径 `/` 保留登录入口、面试入口和登录状态展示。云端历史记录、文件上传、语音或视频面试仍暂缓。
