...
当前版本先验证最小可用闭环。当前已完成基础主页框架、主页封面视觉优化、主页登录流修正、面试工作台信息架构拆分、新面试页面体验优化、本地文本文件导入入口、本地历史记录初始化、列表和详情查看、登录入口页面壳、`/login` 和 `/interview` 路由拆分、Supabase Auth 账号系统 MVP，以及内部测试版上线准备文档。云端历史记录、文件上传、语音或视频面试仍暂缓。
...
- `/interview/new` 初始状态只显示新面试输入和操作入口，生成问题后再展示问题区
...
- 文本导入后仍可继续编辑，并会清空旧问题、回答、评价和保存提示
- 前端空输入校验
...
10. 可分别导入本地 `.txt` / `.md` 到岗位 JD 或个人简历输入区，确认文本填入后仍可编辑；也可以点击 `填入示例 JD/简历` 走快速测试。
11. 点击 `生成面试问题`。
12. 等待问题生成。
13. 点击 `填入测试回答`。
...
16. 点击 `生成最终评价`。
17. 检查最终评价是否完整展示。
18. 检查最终评价区是否提示已保存到本地历史记录。
19. 进入 `/interview/history`，检查是否能看到最近本地历史记录和详情。
20. 回到 `/interview/new` 点击 `开始新一轮`，确认当前输入、问题、回答和评价被清空，但历史记录仍保留。
