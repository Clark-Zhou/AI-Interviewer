...
components/AppEntry.js            首页入口状态切换组件
...
5. 等待问题生成。
6. 点击 `填入测试回答`。
7. 点击 `提交全部回答`，或逐题点击 `提交本题回答`。
8. 确认进度为 `已提交 6 / 6`。
9. 点击 `生成最终评价`。
10. 检查最终评价是否完整展示。
11. 检查最终评价区是否提示已保存到本地历史记录。
12. 检查页面底部历史记录区是否新增记录，点击后是否能查看详情。
13. 点击 `开始新一轮`，确认当前输入、问题、回答和评价被清空，但历史记录仍保留。
