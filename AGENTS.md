...
3. 如果要改开发测试辅助逻辑，阅读 `docs/DEVELOPMENT_TESTING.md`。
4. 如果要改产品范围、用户流程或功能边界，阅读 `docs/PRD.md`。
5. 运行只读检查命令，例如 `git status --short` 和必要的 `rg` / `sed`，确认工作区是否已有用户改动。
6. 如果发现工作区有未提交改动，不要默认撤销；先判断是否与当前任务相关，必要时向用户确认。
...
7. 如果改变开发测试方式、开发辅助按钮、mock 策略或本地测试流程，更新 `docs/DEVELOPMENT_TESTING.md`。
8. 如果改变项目安装、运行、目录结构、环境变量或总体功能说明，更新 `README.md`。
9. 如果改变产品目标、MVP 范围、用户流程或非目标，更新 `docs/PRD.md`。
10. 如果改动影响依赖，说明需要用户自行运行的安装命令，不要直接安装。
11. 如果没有实际运行测试，要在最终回复里说明“未运行测试”和原因。
