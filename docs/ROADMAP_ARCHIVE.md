...
这个文件归档 AI Interview Simulator 已完成阶段的详细方案、任务拆分、验收标准和代码审查关注点。

默认情况下，新 session 不需要阅读本文件。只有需要追溯历史决策、理解旧阶段为什么这样设计，或审查涉及旧阶段行为的复杂回归时，才按需阅读相关章节。

关联文件：

- `docs/ROADMAP.md`：当前阶段计划和下一步优先级，默认优先阅读。
- `docs/PROJECT_STATUS.md`：当前项目事实和已完成功能状态。
- `AGENTS.md`：多 session 协作规则和收尾规范。

## 归档内容

以下内容来自 `docs/ROADMAP.md` 的已完成阶段详情。归档后，ROADMAP 本体应继续保持轻量，避免默认阅读时消耗过多 token。
...
- 新增函数或关键逻辑有简短中文注释。
- 不新增除项目所有者已安装的 `mammoth` 和 `pdf-parse` 以外的新依赖。
...
4. 阶段 13 当时新增 `app/interview/page.js` 挂载模拟面试主界面；阶段 19 后该路径已调整为面试工作台入口。
...
阶段状态：已完成。当前已经把 `/interview` 从完整表单页拆成受保护的面试工作台入口页，并把新面试和本地历史记录拆到独立子路由。

### 阶段目标

- `/interview` 作为受保护的面试工作台入口页。
- 工作台入口页只提供两个主要入口：`开始新的面试` 和 `查看历史记录`。
- `/interview/new` 承载当前完整的新面试流程，包括 JD/简历输入、开发 Mock、生成问题、回答、评价和保存历史。
- `/interview/history` 承载最近本地历史记录列表和详情查看。
- 保持未登录访问 `/interview`、`/interview/new`、`/interview/history` 都会进入 `/login`。
- 保持主页进入 `/interview` 后先看到工作台入口页，而不是直接看到复杂表单。

### 产品思路

这次不是做新的 AI 能力，而是整理用户路径：用户进入面试区后先判断“我要开始一轮新练习”还是“我要复盘历史记录”。这能降低首屏认知负担，也为以后增加云端历史、历史详情页或恢复练习打基础。

### MVP 范围

本阶段建议包含：

- 调整 `app/interview/page.js`，让它渲染工作台入口页。
- 新增 `app/interview/new/page.js`，挂载现有新面试流程。
- 新增 `app/interview/history/page.js`，展示本地历史记录列表和详情。
- 从 `components/InterviewSimulator.js` 中拆出历史记录展示组件，供 `/interview/history` 复用。
- 新面试成功生成最终评价后仍保存到 localStorage，并可在 `/interview/history` 看到。
- 更新相关中文 file header 和关键逻辑注释。
- 更新必要文档和测试路径。

本阶段暂不包含：

- 云端历史记录或数据库。
- 历史记录删除、搜索、编辑或恢复到当前面试。
- 历史详情动态路由，例如 `/interview/history/[id]`。
- 新的 AI 流程、prompt 或 API 协议。
- 大范围视觉重设计或全站组件库重构。
- 文件上传、语音、视频或多轮追问。
...
/interview/history   受保护的本地历史记录
```

说明：`proxy.js` 当前保护 `/interview/:path*`，理论上能覆盖新增子路由；开发 session 仍需确认直接访问子路由时未登录会被拦回 `/login`。

### 任务拆分建议

1. 先阅读 `README.md`、`AGENTS.md`、`docs/PROJECT_STATUS.md`、`docs/ROADMAP.md`、`docs/PRD.md` 和 `docs/DEVELOPMENT_TESTING.md`。
2. 检查当前 `app/interview/page.js`、`components/InterviewSimulator.js`、`components/AuthStatusBar.js`、`lib/client/interviewHistoryStorage.js` 和 `proxy.js`。
3. 新增或调整工作台入口页，让 `/interview` 只展示两个入口和必要账号状态。
4. 新增 `/interview/new` 页面，复用现有 `InterviewSimulator` 的新面试流程。
5. 拆出历史记录组件，例如 `components/InterviewHistoryPanel.js`，把当前历史列表和详情展示从 `InterviewSimulator` 中移出。
6. 新增 `/interview/history` 页面，使用历史记录组件读取 localStorage 并展示最近记录。
7. 如果 `InterviewSimulator` 仍需要在评价成功后提示保存结果，可以保留保存状态提示，但不要再把完整历史区放在新面试页底部。
8. 检查移动端和桌面端的入口页、流程页、历史页是否不遮挡、不溢出。
9. 按 `AGENTS.md` 的分档收尾规则更新必要文档。

### 验收标准

阶段 19 完成时，应满足：

- 已登录用户从主页进入 `/interview` 后看到工作台入口页。
- `/interview` 页面有且只有两个主要入口：`开始新的面试` 和 `查看历史记录`。
- 点击 `开始新的面试` 进入 `/interview/new`，并能完成当前真实 AI 流程和开发 Mock 流程。
- 点击 `查看历史记录` 进入 `/interview/history`，能看到最近本地历史记录和详情。
- 未登录直接访问 `/interview`、`/interview/new` 或 `/interview/history` 都会进入 `/login`。
- 新面试完成最终评价后仍保存本地历史记录。
- 历史记录页能看到新保存的记录；旧历史记录仍能正常展示。
- `/interview/new` 不再把完整历史记录区堆在页面底部，但可以保留保存成功/失败提示和进入历史页的入口。
- 没有改 DeepSeek API、prompt、parser、Supabase Auth、localStorage 数据结构或环境变量。
- 新增重要文件有中文 file header；新增函数和关键逻辑有简短中文注释。

### 代码审查关注点

代码审查 session 应重点检查：

- 路由拆分是否清晰，`/interview` 是否只做工作台入口而不是继续承载完整表单。
- `/interview/new` 是否完整保留真实 AI、Mock、答题、评价、保存历史和错误重试能力。
- `/interview/history` 是否只读 localStorage 历史，不引入云端存储假设。
- 未登录访问三个 `/interview` 相关路由是否都被保护。
- 历史记录组件是否没有在服务端直接访问 `window` 或 `localStorage`。
- 是否没有改变现有历史记录数据结构，旧记录是否仍可展示。
...
阶段状态：已完成。阶段 19 已经完成并审查了 `/interview` 工作台拆分；本阶段在这个基础上优化了 `/interview/new` 页面，不属于阶段 19 的返工。
...
- 在岗位 JD 输入区提供本地文件导入入口。
- 在个人简历输入区提供本地文件导入入口。
- 第一版只支持 `.txt` 和 `.md` 纯文本文件。
- 文件内容在浏览器本地读取后填入现有 textarea。
- 导入后用户仍然可以继续手动编辑文本。
- 不把原始文件对象保存到 localStorage、历史记录或后端。

### 产品思路

这不是完整“上传文件解析系统”，而是一个更轻量的输入辅助：把本来要复制粘贴的文本，从本地文件直接导入到现有文本框。这样既能提升开发和真实使用效率，也不会引入文件存储、后端解析和隐私权限复杂度。

### MVP 范围

本阶段建议包含：

- 给 JD textarea 增加 `导入 JD 文件` 入口。
- 给简历 textarea 增加 `导入简历文件` 入口。
- 使用浏览器端 File API / FileReader 读取 `.txt` 和 `.md`。
- 对不支持的文件类型给出明确提示。
- 对读取失败、空文件、过大文件给出轻量提示。
- 导入成功后清空上一轮问题、回答、提交状态、评价和保存提示，避免新文本与旧问题混用。
- 更新必要中文注释和测试说明。
...
- 多文件合并。
- AI 自动提取字段。
- 把文件对象、文件名或文件内容以外的元数据保存到历史记录。
- 新增除项目所有者已安装解析库以外的依赖。

### 建议实现方式

- 优先在 `components/InterviewSimulator.js` 内实现小范围文件导入逻辑，或抽出轻量 client helper。
- 文件 input 可以使用原生 `<input type="file">`，限制 `accept=".txt,.md,text/plain,text/markdown"`。
- 读取文件前检查扩展名和 MIME type，但不要只依赖 MIME type，因为不同系统可能返回空字符串。
- 建议限制文件大小，例如 200KB 到 500KB 之间，避免误导入超大文件导致页面卡顿。
- 读取成功后把文本写入对应的 `jobInfo` 或 `resume` state。
- 读取成功后复用当前“输入变化后清空错误”的逻辑，并清空已生成问题和评价状态。

### 任务拆分建议

1. 先阅读 `README.md`、`AGENTS.md`、`docs/PROJECT_STATUS.md`、`docs/ROADMAP.md`、`docs/PRD.md` 和 `docs/DEVELOPMENT_TESTING.md`。
2. 检查 `components/InterviewSimulator.js` 当前输入区、状态清空逻辑和开发辅助按钮。
3. 为 JD 和简历分别增加文件选择入口，不影响现有手动粘贴。
4. 实现浏览器本地读取 `.txt` / `.md` 内容并填入对应 textarea。
5. 处理不支持类型、空文件、读取失败和过大文件提示。
6. 导入成功后清空旧问题、回答、评价和保存提示，但不要删除 localStorage 历史记录。
7. 确认真实 AI、Mock、答题、评价、保存历史和错误重试流程仍可用。
8. 按 `AGENTS.md` 的分档收尾规则更新必要文档。
...
- `/interview/new` 的岗位 JD 输入区可以选择 `.txt` 或 `.md` 文件并填入岗位信息 textarea。
- `/interview/new` 的个人简历输入区可以选择 `.txt` 或 `.md` 文件并填入简历 textarea。
- 导入后用户可以继续手动编辑文本。
- 导入成功后旧问题、回答、提交状态、最终评价和保存提示被清空，避免旧结果和新输入混用。
- 不支持的文件类型会提示用户使用 `.txt` 或 `.md`。
- 空文件、读取失败或超过大小限制时有明确提示，不影响当前页面继续使用。
- 文件对象不保存到 localStorage，不发送到后端，不进入历史记录。
- 历史记录仍只保存最终 JD 文本、简历文本、问题、回答和评价。
- 真实 AI、Mock、答题、评价、保存历史和错误重试流程仍可用。
- 没有新增后端上传 API；除项目所有者已安装的 `mammoth` / `pdf-parse` 外，没有再新增依赖；没有改 DeepSeek API、prompt、parser、Supabase Auth 或 localStorage 数据结构。

### 代码审查关注点

- 文件导入是否完全在浏览器本地完成，没有调用后端上传接口。
- 是否只支持 `.txt` / `.md`，并对 PDF、DOCX 等不支持类型给出清楚提示。
- 文件大小、空文件和读取失败是否有处理。
- 导入成功后是否清空旧问题和旧评价，避免用户误用旧结果。
- 是否没有把 File 对象、文件名、路径、token 或其他敏感信息写入历史记录或日志。
- 是否没有新增除 `mammoth` / `pdf-parse` 以外的依赖或改变 AI/API/Auth/历史数据结构。
- 手动粘贴、开发 Mock、真实 AI 和历史保存是否仍可用。
...
项目所有者已安装本阶段需要的依赖，开发 session 不需要也不应该重复安装：
...
如果开发 session 发现依赖版本异常，应先向用户说明情况，不要自行执行 `npm install`。

### 阶段目标

- 支持在岗位 JD 输入区导入 `.pdf` / `.docx`，解析后填入现有 textarea。
- 支持在个人简历输入区导入 `.pdf` / `.docx`，解析后填入现有 textarea。
- 新增项目后端解析接口，前端只调用项目自己的 API。
- 文件只用于一次性解析，不保存原始文件。
- 解析结果仍然是普通文本，用户可以继续手动编辑。
- 保留阶段 21 已有 `.txt` / `.md` 本地导入能力。

### 产品思路

本阶段仍然不是完整“文件管理系统”。它只是把用户手里的常见 PDF / DOCX 文档转换成输入框文本，降低复制粘贴成本。对用户来说，最终进入 AI 提问和评价流程的仍是 textarea 里的文本，而不是文件本身。

### MVP 范围

本阶段建议包含：

- 新增 `/api/parse-document` 或同等后端 API route。
- 前端为 JD 和简历导入入口支持 `.pdf` / `.docx`。
- 后端使用 `mammoth` 解析 `.docx`。
- 后端使用 `pdf-parse` 解析文本型 `.pdf`。
- 解析成功后返回纯文本给前端。
- 前端把纯文本填入对应 textarea，并清空旧问题、回答、提交状态、评价和保存提示。
- 对文件类型不支持、文件过大、解析失败、解析为空给出明确提示。
- 更新必要中文 file header、函数/代码块注释和测试说明。
...
- 数据库保存文件。
- AI 自动结构化简历或 JD。
- 修改 DeepSeek prompt、parser 或请求协议。
- 修改 Supabase Auth。
- 修改 localStorage 历史记录数据结构。
- 新增除项目所有者已安装解析库以外的依赖。

### 建议实现方式

- 建议新增 server route：`app/api/parse-document/route.js`。
- route 使用 `request.formData()` 接收单个文件和目标字段，例如 `target: jobInfo | resume`。
- route 只接受 `.pdf` / `.docx`，并设置大小限制，例如 2MB 到 5MB。
- `.docx` 使用 `mammoth.extractRawText` 提取纯文本。
- `.pdf` 使用 `pdf-parse` 提取文本型 PDF 内容。
- 解析后 trim 文本；如果为空，提示用户该文件可能是扫描件或图片型 PDF，建议复制粘贴或换文件。
- 前端调用项目自己的解析 API，不直接在前端引入 `mammoth` 或 `pdf-parse`。
- 前端收到纯文本后写入 `jobInfo` 或 `resume` state，并复用清空旧面试状态的逻辑。
- 不要把文件对象、文件名、文件路径或解析前原始文件内容写入 localStorage、历史记录或日志。

### 任务拆分建议

1. 先阅读 `README.md`、`AGENTS.md`、`docs/PROJECT_STATUS.md`、`docs/ROADMAP.md`、`docs/PRD.md` 和 `docs/DEVELOPMENT_TESTING.md`。
2. 检查 `package.json`，确认 `mammoth` 和 `pdf-parse` 已存在，不要安装依赖。
3. 检查阶段 21 的文件导入实现，理解 `.txt` / `.md` 是如何填入 JD 和简历 textarea 的。
4. 新增后端解析 route，并写中文 file header。
5. 在 client API 层新增解析文档的封装，让前端组件只调用项目内部 API。
6. 在 `/interview/new` 文件导入入口里接入 `.pdf` / `.docx` 解析流程，保留 `.txt` / `.md` 本地读取流程。
7. 增加解析 loading、错误提示和空内容提示。
8. 导入成功后清空旧问题、回答、评价和保存提示，但不删除 localStorage 历史记录。
9. 运行必要检查，至少做 `git diff --check` 和 `npm run build`；不要启动 dev server，除非用户明确要求。
10. 按 `AGENTS.md` 的分档收尾规则更新必要文档。

### 验收标准

阶段 22 完成时，应满足：

- `/interview/new` 的岗位 JD 输入区可以导入文本型 `.pdf` 并填入 textarea。
- `/interview/new` 的岗位 JD 输入区可以导入 `.docx` 并填入 textarea。
- `/interview/new` 的个人简历输入区可以导入文本型 `.pdf` 并填入 textarea。
- `/interview/new` 的个人简历输入区可以导入 `.docx` 并填入 textarea。
- 阶段 21 的 `.txt` / `.md` 导入能力仍可用。
- 解析成功后用户仍可继续手动编辑文本。
- 解析成功后旧问题、回答、提交状态、最终评价和保存提示被清空。
- 文件过大、类型不支持、解析失败、解析为空都有明确提示。
- 扫描版 PDF 或图片型 PDF 无法解析时，提示用户改用复制粘贴或换文本型文件。
- 前端只调用项目内部 `/api/parse-document`，不直接引入解析库。
- 原始文件不保存到 localStorage，不进入历史记录，不写入数据库。
- 历史记录仍只保存最终 JD 文本、简历文本、问题、回答和评价。
- 真实 AI、Mock、答题、评价、保存历史和错误重试流程仍可用。
- 除项目所有者已安装的 `mammoth` / `pdf-parse` 外，没有再新增依赖；没有改 DeepSeek API、prompt、parser、Supabase Auth 或 localStorage 数据结构。

### 代码审查关注点

- API route 是否只做一次性解析，没有保存文件。
- 文件类型和大小限制是否在前后端都有清晰处理，后端不能只信任前端。
- `.docx` 是否用 `mammoth`，文本型 `.pdf` 是否用 `pdf-parse`，没有把解析库打进前端 bundle。
- 解析为空时是否有针对扫描件/图片型 PDF 的友好提示。
- 解析成功后是否清空旧问题和旧评价，避免新输入复用旧结果。
- 是否没有把 File 对象、文件名、路径或原始文件内容写入历史记录或日志。
