...
1. 打开生产根路径，确认会看到基础主页。
2. 在未登录状态下点击主页的面试入口，确认会进入 `/login`。
3. 用测试邮箱注册账号。
4. 如果 Supabase 要求邮箱确认，按邮件提示完成确认后再登录。
5. 登录成功后确认可以进入 `/interview`。
6. 刷新 `/interview`，确认仍保持登录态。
7. 登出后确认回到 `/login`。
8. 未登录直接访问 `/interview`，确认会回到 `/login`。
9. 回到主页，确认主页显示未登录状态。
10. 再次登录后回到主页，确认主页显示当前账号信息。
11. 从主页进入 `/interview`，输入岗位 JD 和简历。
12. 点击真实 AI 生成问题，确认能拿到 6 道结构化问题。
13. 填写回答并提交全部回答。
14. 生成最终评价，确认总分、总结、优势、风险点、建议和逐题反馈完整展示。
15. 确认最终评价保存到当前浏览器本地历史记录。
16. 点击历史记录，确认详情可以查看。
17. 点击开始新一轮，确认当前面试状态清空但历史记录保留。
18. 确认生产环境不展示 `填入示例 JD/简历`、`填入测试回答`、`使用 Mock 问题`、`使用 Mock 评价` 等开发辅助按钮。
