...
```text
在 `develop-improve-efficiency` 分支完成 docs/ROADMAP.md 的阶段 9：一键提交全部回答。只新增答题提交效率能力，不改 AI API、prompt、mock 流程或历史保存逻辑。
```
...
16. 阶段 9 完成后，填写全部回答并点击 `提交全部回答`，是否一次性更新所有题目的提交状态。
17. 阶段 9 完成后，如果仍有空回答，点击 `提交全部回答` 是否提示先填写所有回答。

后续可以优先考虑：

- 完成一键提交全部回答，并补充对应测试记录。
- 为最终评价失败增加重试按钮。
- 优化最终评价 prompt 的评分标准。
- 补充更正式的 PR 描述和测试记录。
