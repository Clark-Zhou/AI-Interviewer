...
```text
在 `develop-improve-efficiency` 分支完成 docs/ROADMAP.md 的阶段 11：错误恢复与文案小修。只新增生成问题失败和最终评价失败后的重试入口，并小范围优化用户可见文案；不改 AI API、prompt、mock 流程或历史保存逻辑。
```
...
20. 阶段 11 完成后，生成问题失败时是否显示 `重试生成问题`，且重试复用当前 JD 和简历。
21. 阶段 11 完成后，最终评价失败时是否显示 `重试生成评价`，且重试复用当前已提交问答。
22. 阶段 11 完成后，关键错误、空状态和保存提示文案是否清楚、不误导用户。

后续可以优先考虑：

- 完成错误恢复与文案小修，并补充对应测试记录。
- 优化最终评价 prompt 的评分标准。
- 补充更正式的 PR 描述和测试记录。
