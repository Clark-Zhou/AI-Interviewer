...
这个阶段暂时不做真实登录、数据库、文件上传和云端历史记录，先验证最核心的面试练习闭环是否成立。目前已经完成本地历史记录初始化、列表和详情查看，用于保存并复盘完整面试 session。当前分支已完成登录入口页面壳和 `/login`、`/interview` 前端路由拆分，用于改善第一屏体验和预留未来账号入口，不实现真实账号系统。
...
当前已经完成的是 MVP 的核心闭环、开发效率优化、登录入口页面壳和入口/主界面路由拆分。当前分支还未实现真实登录或账号系统：

```text
访问根路径后进入 /login
用户打开 /login 后先看到登录入口页面壳
用户填写体验版邮箱和口令
点击进入体验版后进入 /interview 模拟面试主界面
浏览器返回键可从 /interview 回到 /login
输入岗位信息
输入个人简历
点击生成面试问题
前端调用后端生成问题 API
...
登录入口页面路由。挂载 `components/LoginEntryShell.js`，不做真实登录或鉴权。

```text
app/interview/page.js
```

模拟面试主界面路由。挂载 `components/InterviewSimulator.js`，当前不做访问保护。

```text
components/LoginEntryShell.js
```

登录入口页面壳。展示背景视觉、悬浮体验框、邮箱输入、体验口令输入和 `进入体验版` 按钮；不调用登录 API，不保存密码。
...
```text
在 `develop-login-page` 分支做阶段 13 后的人工回归测试和代码审查。重点确认 `/login` 和 `/interview` 导航清晰，浏览器返回键能从主界面回到入口页，同时不引入真实账号系统。
```

建议检查：

1. 访问 `/` 是否进入 `/login`。
2. 访问 `/login` 是否看到登录入口页面壳。
3. 入口页是否有背景视觉和悬浮体验框，桌面和移动端不遮挡、不溢出。
4. 入口主按钮是否使用 `进入体验版`，且页面没有暗示真实账号已经可用。
5. 邮箱或体验口令为空时，点击入口按钮是否只显示前端提示。
6. 点击 `进入体验版` 后是否进入 `/interview` 模拟面试主界面。
7. 在 `/interview` 点击浏览器返回键是否回到 `/login`。
8. 岗位信息为空时，点击生成问题是否提示错误。
9. 简历为空时，点击生成问题是否提示错误。
10. 正常输入岗位和简历后，是否可以生成问题。
11. 每道题未填写回答时，点击提交是否提示错误。
12. 每道题填写并提交后，答题进度是否正确更新。
13. 所有题提交后，是否可以生成最终评价。
14. 修改某道题回答后，旧的提交状态和旧评价是否被清空。
15. 最终评价成功后，页面是否提示已保存到本地历史记录。
16. localStorage 的 `ai-interview-sessions` 中是否新增完整 session，并且最新记录排在最前面。
17. 多次完成评价后，本地历史是否最多保留 10 条。
18. 页面底部历史记录区是否显示最新记录。
19. 点击历史记录后，是否能看到岗位摘要、简历摘要、整体评价和问答记录。
20. 开发环境下 `使用 Mock 问题` 是否不调用后端 API，并能进入答题流程。
21. 开发环境下 `使用 Mock 评价` 是否不调用后端 API，并能生成最终评价。
22. Mock 评价成功后是否新增本地历史记录，并显示 Mock 来源标记。
23. 填写全部回答并点击 `提交全部回答`，是否一次性更新所有题目的提交状态。
24. 如果仍有空回答，点击 `提交全部回答` 是否提示先填写所有回答。
25. 点击 `开始新一轮` 是否清空当前输入、问题、回答、提交状态、评价和提示信息。
26. 本地历史记录是否仍然保留，历史列表和详情是否不被删除。
27. 生成问题失败时是否显示 `重试生成问题`，且重试复用当前 JD 和简历。
28. 最终评价失败时是否显示 `重试生成评价`，且重试复用当前已提交问答。
29. 关键错误、空状态和保存提示文案是否清楚、不误导用户。
