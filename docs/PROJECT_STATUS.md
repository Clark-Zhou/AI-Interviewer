...
MVP 产品需求文档，描述产品目标、用户流程、功能范围和未来方向。该文件不是通用必读；只有产品目标、用户主流程、MVP 边界或非目标变化时再阅读或更新。
...
当前阶段计划、下一步优先级和验收标准文档，用于对齐产品助理、阶段开发、代码审查、开发测试等不同 session 的工作顺序。已完成阶段详情不应长期堆在这里。

```text
docs/ROADMAP_ARCHIVE.md
```

已完成历史阶段详情归档。该文件不是通用必读；只有追溯旧阶段设计、验收标准或审查关注点时按需阅读。
...
阶段 22 已完成、审查并合并到 `main`。当前建议先由产品助理 session 讨论下一阶段方向，再从 `main` 新建对应功能分支开发。

建议下一步讨论：

1. 是否继续优化已有面试流程体验。
2. 是否优先改善历史记录能力，例如删除、搜索或恢复历史 session。
3. 是否开始规划云端历史记录和数据库存储。
4. 是否保持当前文件解析边界，暂不做 OCR、图片识别或云端文件存储。
5. 新阶段确定后，再更新 `docs/ROADMAP.md` 的阶段计划、任务拆分和验收标准。
