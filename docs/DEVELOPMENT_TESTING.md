...
17. 点击 `填入测试回答`。
18. 点击 `提交全部回答`，或逐题点击 `提交本题回答`。
19. 确认答题进度显示为 `已提交 6 / 6`。
20. 点击真实 AI 评价按钮，等待 DeepSeek 返回最终评价。
21. 检查页面是否展示总分、总结、优势、风险点、改进建议、逐题反馈和后续练习题。
22. 检查最终评价区是否显示 `最终评价已生成，并已保存到本地历史记录。`。
23. 进入 `/interview/history`，检查最近本地历史记录是否新增，点击后能看到详情。
24. 回到 `/interview/new` 点击 `开始新一轮`，确认当前岗位 JD、简历、问题、回答、提交状态、评价和提示信息被清空，历史记录仍保留。
...
11. 点击 `填入测试回答`。
12. 点击 `提交全部回答`，应立即显示 `已提交 6 / 6`。
13. 点击 `使用 Mock 评价`，应立即展示固定结构的最终评价。
14. 打开浏览器 DevTools，确认 localStorage 的 `ai-interview-sessions` 中新增一条完整记录，且 `generationSource` 标记为 mock。
15. 进入 `/interview/history`，检查历史记录页是否新增记录，点击后能看到详情。
16. 回到 `/interview/new` 点击 `开始新一轮`，确认页面回到初始输入状态，历史记录仍保留。
...
以下检查点用于验证阶段 22 完成后的 PDF/DOCX 文档解析入口。

PDF/DOCX 解析完成后，重点检查：

- 岗位 JD 输入区可以选择文本型 `.pdf` 文件并填入 textarea。
- 岗位 JD 输入区可以选择 `.docx` 文件并填入 textarea。
- 个人简历输入区可以选择文本型 `.pdf` 文件并填入 textarea。
- 个人简历输入区可以选择 `.docx` 文件并填入 textarea。
- 阶段 21 已有 `.txt` / `.md` 导入能力仍可用。
- 解析成功后用户仍然可以继续手动编辑文本。
- 解析成功后旧问题、回答、提交状态、最终评价和保存提示会被清空。
- 不支持的文件类型、文件过大、解析失败和解析为空时有明确提示。
- 扫描版 PDF 或图片型 PDF 无法解析时，应提示用户改用复制粘贴或换文本型文件。
- 前端只调用项目内部解析 API，不直接引入 `mammoth` 或 `pdf-parse`。
- 原始文件、文件名、文件路径或其他文件元数据不会被保存到历史记录、localStorage、数据库或日志。
- 真实 AI、开发 Mock、答题、评价、保存历史和错误重试流程仍可用。
- 除项目所有者已安装的 `mammoth` / `pdf-parse` 外，不应新增其他依赖。
- 云端文件存储、OCR、图片识别和 AI 自动结构化仍不属于本阶段。
- 当前只验证业务层 5MB 文件大小提示；`request.formData()` 之前的超大请求体拦截暂未实现，后续需要在平台或更底层请求体限制中处理。
