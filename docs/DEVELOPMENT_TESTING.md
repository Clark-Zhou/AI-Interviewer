...
14. 点击 `填入示例 JD/简历`。
15. 点击真实 AI 问题按钮，等待 DeepSeek 返回问题列表。
16. 点击 `填入测试回答`。
17. 点击 `提交全部回答`，或逐题点击 `提交本题回答`。
18. 确认答题进度显示为 `已提交 6 / 6`。
19. 点击真实 AI 评价按钮，等待 DeepSeek 返回最终评价。
20. 检查页面是否展示总分、总结、优势、风险点、改进建议、逐题反馈和后续练习题。
21. 检查最终评价区是否显示 `最终评价已生成，并已保存到本地历史记录。`。
22. 进入 `/interview/history`，检查最近本地历史记录是否新增，点击后能看到详情。
23. 回到 `/interview/new` 点击 `开始新一轮`，确认当前岗位 JD、简历、问题、回答、提交状态、评价和提示信息被清空，历史记录仍保留。
...
8. 点击 `填入示例 JD/简历`。
9. 点击 `使用 Mock 问题`，应立即展示固定问题列表。
10. 点击 `填入测试回答`。
11. 点击 `提交全部回答`，应立即显示 `已提交 6 / 6`。
12. 点击 `使用 Mock 评价`，应立即展示固定结构的最终评价。
13. 打开浏览器 DevTools，确认 localStorage 的 `ai-interview-sessions` 中新增一条完整记录，且 `generationSource` 标记为 mock。
14. 进入 `/interview/history`，检查历史记录页是否新增记录，点击后能看到详情。
15. 回到 `/interview/new` 点击 `开始新一轮`，确认页面回到初始输入状态，历史记录仍保留。
...
说明：阶段 14 当时登录成功后直接进入 `/interview`。阶段 18 之后，通用推荐流程以“登录或注册成功后回到 `/`，再从主页进入 `/interview`”为准。

- `.env.local` 已配置 `NEXT_PUBLIC_SUPABASE_URL` 和 `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`，且没有提交真实环境变量。
- 新邮箱可以注册。
- 已注册邮箱可以登录。
- 登录成功后回到 `/`，再从主页进入 `/interview`。
- 未登录直接访问 `/interview` 时会回到 `/login`。
- 已登录时刷新 `/interview` 仍保持登录态。
- 主界面登出后回到 `/login`；主页登出后停留在 `/` 并回到未登录状态；再次访问 `/interview` 会被拦回登录页。
...
- 已登录时主页把登录入口替换为可用的 `登出`。
- `/login` 注册/登录、`/interview` 面试主流程、主页登出、主界面登出和登录态保持仍可用。
...
以下检查点用于验证阶段 18 完成后的主页登录流。

主页登录流修正完成后，重点检查：

- 访问 `/` 显示主页，不应自动进入 `/login` 或 `/interview`。
- 未登录状态下，主页显示未登录状态、登录入口和面试入口。
- 未登录点击主页面试入口进入 `/login`。
- `/login` 登录或注册成功后回到 `/`。
...
以下检查点用于验证阶段 19 完成后的 `/interview` 信息架构拆分。

面试工作台拆分完成后，重点检查：

- 已登录用户从主页进入 `/interview` 后看到工作台入口页，而不是直接看到完整面试表单。
- `/interview` 页面只提供 `开始新的面试` 和 `查看历史记录` 两个主要入口。
- 点击 `开始新的面试` 进入 `/interview/new`。
- `/interview/new` 中真实 AI 生成问题、Mock 问题、填入测试回答、提交全部回答、真实 AI 评价、Mock 评价、错误重试和保存历史都仍可用。
- 点击 `查看历史记录` 进入 `/interview/history`。
- `/interview/history` 能展示最近本地历史记录，空历史时有空状态，有历史时可查看详情。
- 新面试完成最终评价后，进入 `/interview/history` 能看到新保存的记录。
- 未登录直接访问 `/interview`、`/interview/new` 或 `/interview/history` 都会进入 `/login`。
- 历史记录页只读取浏览器 localStorage，不假装有云端同步或跨设备历史。
- 桌面和移动端下三个页面都不遮挡、不溢出，入口按钮可点击。


## 阶段 20 检查点

以下检查点用于验证阶段 20 完成后的 `/interview/new` 新面试页面体验。

新面试页优化完成后，重点检查：

- `/interview/new` 初始状态不显示历史记录列表或历史详情。
- `/interview/new` 初始状态不显示 `模拟问题` 面板，也不显示问题空状态文案。
- 点击真实 AI 生成问题后，生成中状态或问题列表会出现。
- 开发环境点击 `使用 Mock 问题` 后，问题列表会出现。
- 真实 AI 生成问题、Mock 问题、填入测试回答、提交全部回答、真实 AI 评价、Mock 评价、错误重试和保存历史仍可用。
- 最终评价成功后可以通过轻量入口前往 `/interview/history` 查看历史记录。
- `/interview/history` 的历史记录列表和详情不受影响。
- `/interview`、`/interview/new`、`/interview/history` 的未登录保护不受影响。
- `/interview/new` 视觉延续青色主色调和轻量渐变，但没有引入复杂视觉重构或不可用入口。


## 阶段 21 检查点

以下检查点用于验证阶段 21 完成后的文本文件导入入口。

文本文件导入完成后，重点检查：

- 岗位 JD 输入区可以选择 `.txt` 文件并填入 textarea。
- 岗位 JD 输入区可以选择 `.md` 文件并填入 textarea。
- 个人简历输入区可以选择 `.txt` 文件并填入 textarea。
- 个人简历输入区可以选择 `.md` 文件并填入 textarea。
- 导入后用户仍然可以继续手动编辑文本。
- 导入成功后旧问题、回答、提交状态、最终评价和保存提示会被清空。
- 导入文件不会删除 localStorage 中已经保存的历史记录。
- 不支持的文件类型会提示用户使用 `.txt` 或 `.md`。
- 空文件、读取失败和超过大小限制时有明确提示，页面不会卡死或清空有效内容。
- 文件对象、文件名、文件路径或其他文件元数据不会被保存到历史记录。
- 真实 AI、开发 Mock、答题、评价、保存历史和错误重试流程仍可用。
- PDF、DOCX、云端上传、文件存储和 AI 自动解析仍不属于本阶段。
